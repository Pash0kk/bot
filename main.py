...
def get_continue() -> ReplyKeyboardMarkup:
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add(KeyboardButton(text='Продовжити'))
...
async def abort_btn(message: types.Message, state: FSMContext):
    if state is None:
        return
    
    await state.finish()
    await message.answer("Ви відмінили замовлення!", reply_markup=get_continue())
...
@dp.message_handler(Text(equals='Продовжити'), state=None)
async def start_command(message: types.Message):
    await ClientStateGroup.main_state.set()
    await message.answer("Ви повернулися у головне меню.", reply_markup=get_main_keyboard())
...
async def check_date(message: types.Message, state: FSMContext):
    ...
    await message.answer("Оберіть дату виїзду", reply_markup=get_abort())
